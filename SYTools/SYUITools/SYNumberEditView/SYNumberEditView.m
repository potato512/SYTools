...
- (void)buttonReduceClick
{
    ...
    number--;
    if (number < self.numberLimit)
    {
        [SYIToast alertWithTitleCenter:[NSString stringWithFormat:@"此商品%ld件起购", self.numberLimit]];
        return;
    }
    if (number > 0)
    {
...
- (void)textEdictChange:(NSNotification *)notification
{
    ...
    if (number < self.numberLimit && self.numberLimit >= 0)
    {
        number = self.numberLimit;
    }
    numberText = [NSString stringWithFormat:@"%@", @(number)];
    self.textField.text = numberText;
}
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    if (self.numberEdit)
    {
        self.numberEdit([textField.text integerValue]);
    }
}
